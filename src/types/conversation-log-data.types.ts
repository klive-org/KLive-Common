...
export interface IAssignmentData {
  ...
  assignmentCode: string;
  agentId: string;
  assignedById?: string; // null for self-assigned
  assignedAt: string;
  scenarioId?: string;
  isFirstResponse?: boolean;
  responseDuration?: number; // ms
}
...
export interface IPreviousAssignmentInfo {
  ...
  assignmentCode: string;
  agentId: string;
  assignedAt: string; // When it was first assigned
  endedAt: string; // When transfer happened
  handleDuration: number; // ms
  endReason: string; // e.g., "transferred"
}
...
export interface IConversationTransferredData {
  ...
  transferReason: string; // e.g., "llm_empty_response", "llm_error"
}
...
export type ConversationLogData =
  | IConversationCreatedData
  | IConversationRestartedData
  | IAssignmentData
  | IMissedAssignmentData
  | IConversationEndedData
  | IConversationTransferredData
  | IConversationTransferredToQueueData
  | IAgentActivityData
  | IAgentSendMessageData
  | IVisitorSendMessageData;
